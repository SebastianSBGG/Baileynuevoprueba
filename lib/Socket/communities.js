...
const makeCommunitiesSocket = (config) => {
    ...
    async function parseGroupResult(node) {
        ...
        if (groupNode) {
            try {
                ...
                sock.logger?.error?.({ error }, 'Error al parsear metadata de grupo de comunidad');
                return null;
            }
        }
        return null;
    ...
    return {
        ...
        communityUnlinkGroup: async (groupJid, parentCommunityJid) => {
            await communityQuery(parentCommunityJid, 'set', [
                { tag: 'unlink', attrs: { unlink_type: 'sub_group' }, content: [{ tag: 'group', attrs: { jid: groupJid } }] }
            ]);
        },
        ...
        communityParticipantsUpdate: async (jid, participants, action) => {
            const result = await communityQuery(jid, 'set', [
                { tag: action, attrs: {}, content: participants.map(jid => ({ tag: 'participant', attrs: { jid } })) }
            ]);
...
const extractCommunityMetadata = (result) => {
    ...
    const metadata = {
        ...
        memberAddMode,
        participants: (0, WABinary_1.getBinaryNodeChildren)(community, 'participant').map(({ attrs }) => ({ id: attrs.jid, admin: attrs.type || null })),
