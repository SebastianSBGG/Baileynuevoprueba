            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        ...
                        serverMessageId: 0,
                        newsletterName: "Mancos Y Asociados",
                        contentType: "UPDATE",
                    }
                }
            ]
...
/**
 * Construye los `additionalNodes` requeridos para que mensajes interactivos/de
 * botones/listas/group-story se rendericen de verdad en el cliente del receptor.
 * El servidor de WhatsApp usa estos nodos binarios (<biz><interactive .../></biz>,
 * <biz><list .../></biz>, <bot biz_bot="1"/> en chats 1:1, <meta is_vsn2="1"/> para
 * group stories) para decidir cómo tratar el mensaje — sin ellos el mensaje se
 * entrega igual, pero se renderiza vacío/invisible en la mayoría de clientes, o
 * (para group stories) nunca aparece como story para el resto del grupo.
 *
 * Portado de vinzsocket/Baileys 3.2.8-3.2.10 (fix real: status de grupo que no
 * aparecía para otros miembros).
 *
 * @param {string} jid - jid destino (para detectar grupo vs 1:1)
 * @param {object} m - objeto de contenido del mensaje, ej. { interactiveMessage } o
 *                      ya desenvuelto de un viewOnceMessage (pasar el mensaje interno).
 * @returns {Array<object>} additionalNodes para pasar a relayMessage()
 */
const buildInteractiveAdditionalNodes = (jid, m) => {
    var _a, _b, _c;
    if (!m || typeof m !== 'object')
        return [];
    const inner = ((_a = m.viewOnceMessage) === null || _a === void 0 ? void 0 : _a.message) || ((_b = m.viewOnceMessageV2) === null || _b === void 0 ? void 0 : _b.message) || m;
    const nodes = [];
    const group = (0, WABinary_1.isJidGroup)(jid);
    ...
    else if (inner.interactiveMessage || inner.buttonsMessage || inner.templateMessage || inner.productMessage) {
        const isCarousel = !!((_c = inner.interactiveMessage) === null || _c === void 0 ? void 0 : _c.carouselMessage);
        nodes.push({
            tag: 'biz',
            attrs: {},
            content: [{
                    ...
                    content: [{ tag: 'native_flow', attrs: { name: isCarousel ? 'carousel' : 'mixed', v: '9' } }]
                }]
        });
    }
    else if (inner.groupStatusMessageV2) {
        if (group)
            nodes.push({ tag: 'meta', attrs: { is_vsn2: '1' } });
    }
    if (nodes.length > 0 && !group) {
        nodes.push({ tag: 'bot', attrs: { biz_bot: '1' } });
    }
    return nodes;
};
exports.buildInteractiveAdditionalNodes = buildInteractiveAdditionalNodes;
